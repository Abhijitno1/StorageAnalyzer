...
namespace FilesHunter
{
    ...
    public partial class frmFolderBrowser : Form
    {
        ...
        {
            InitializeComponent();
            this.Load += frmFolderBrowser_Load;
        }

        void frmFolderBrowser_Load(object sender, EventArgs e)
        {
            ofdFileLocation.InitialDirectory = Application.ExecutablePath;
        }

        private void btnOpenDialog_Click(object sender, EventArgs e)
        {
            ofdFileLocation.CheckFileExists = true;
            var dlgResult = ofdFileLocation.ShowDialog();
            if (dlgResult != DialogResult.Cancel)
            {
                txtFileLocation.Text = ofdFileLocation.FileName;
            }
        }
        ...
        private void btnLoadTreeview_Click(object sender, EventArgs e)
        {
            ...
            browser.InputFilePathName = txtFileLocation.Text.Trim();
        ...
        private void DisplayResults(List<string> result)
        {
            if (result.Any())
            {
                foreach (var entry in result)
                {
                    ...
                    //ShowSearchResultsInTreeview(entry);                    
                }
            }
            else
            {
